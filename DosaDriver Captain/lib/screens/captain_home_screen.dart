...
class _CaptainHomeScreenState extends State<CaptainHomeScreen>
    ...
    try {
      ...
      setState(() {
        _openRides = rides.where((r) {
          ...
          if (_captainType != null && rideType != _captainType) {
            return false; // ❌ ignore wrong ride type
          }
      ...
      debugPrint('Failed to fetch open rides: $e');
    }
  }
  ...
  Future<void> _initCaptain() async {
    ...
    await _loadCaptainType();   // MUST run first

    await _loadOnlineState();   // polling starts after captainType exists

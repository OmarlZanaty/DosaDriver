...
class _CaptainRideMapScreenState extends State<CaptainRideMapScreen> {
  ...
  // FIX: Battery-efficient location — Adaptive accuracy
  void _startLocationTracking() {
    ...
    _locationSub = Geolocator.getPositionStream(
      locationSettings: const LocationSettings(
        ...
        distanceFilter: 10, // only update if moved 10m — saves battery
      ),
    ).listen((pos) {
      if (!mounted) return;
      setState(() => _captainLatLng = LatLng(pos.latitude, pos.longitude));
      _updateFirestoreLive(pos.latitude, pos.longitude);
      _maybeDrawRoute();
    });
  }

  Future<void> _updateFirestoreLive(double lat, double lng) async {
  ...
  void _rebuildMarkers() {
    ...
    if (_pickupLatLng != null && _status.index < RideStatus.started.index) {
      markers.add(Marker(
